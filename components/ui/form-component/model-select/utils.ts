...
export const getColorClasses = (color: string, isSelected: boolean = false, isGroupHeader: boolean = false) => {
    ...
    // For non-selected group headers, we might want a neutral hover or specific styling if different from individual items
    if (isGroupHeader) {
        return color === "purple"
            ? `${baseClasses} text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100/70 dark:hover:bg-indigo-900/40`
            : `${baseClasses} text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100/70 dark:hover:bg-neutral-800/50`;
    }
    return color === "purple"
...
export const getTagClass = (type: 'context' | 'tier' | 'capability' | 'owner' | 'apiProvider'): string | ((plan: 'free' | 'pro') => string) => {
    ...
    if (type === 'tier') {
        return (plan: 'free' | 'pro'): string => plan === 'pro'
