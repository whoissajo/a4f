// components/text-fade-animation.tsx
import React, { useEffect, useState, useRef } from 'react';
import { motion } from 'framer-motion';
import { MarkdownRenderer } from '@/components/markdown';

interface TextFadeAnimationProps {
  content: string;
  isStreaming: boolean;
  modelId?: string; // Not used but kept for compatibility
}

/**
 * TextFadeAnimation provides a smooth fade-in animation for streaming text
 * that dynamically adapts to the actual streaming pattern in real-time.
 */
export const TextFadeAnimation: React.FC<TextFadeAnimationProps> = ({ 
  content, 
  isStreaming
}) => {
  ...
  // Fully dynamic adaptive animation speed based on actual streaming patterns
  const getRevealSpeed = () => {
    ...
    const recentTimestamps = chunkTimestampsRef.current.slice(-5); // Last 5 chunks for better pattern detection
    ...
    if (recentTimestamps.length >= 2) {
      ...
      // Calculate average and minimum time between chunks
      ...
      // Calculate ideal chars per second so current chunk finishes just as next chunk arrives
      // This is the key to making it smooth between chunks
      ...
      if (isVeryFastStream) {
        // Very fast streaming - keep up but still maintain some smoothness
        charsPerSecond = Math.min(idealCharsPerSecond, Math.max(60, avgChunkSize / (avgTimeBetweenChunks / 1000) * 0.7));
      } else if (isModerateStream) {
        // Moderate speed - balance between keeping up and smooth animation
        charsPerSecond = Math.min(idealCharsPerSecond, Math.max(40, avgChunkSize / (avgTimeBetweenChunks / 1000) * 0.6));
      } else {
        // Slower streaming - focus on smooth animation
        charsPerSecond = Math.min(idealCharsPerSecond, Math.max(30, avgChunkSize / (avgTimeBetweenChunks / 1000) * 0.5));
      }
      
      // Critical adjustment: if no chunks for a while, assume we're close to the end
      // and speed up to finish displaying what we have
      if (timeSinceLastChunk > 500 && remainingChars > 0) {
        // Accelerate to finish displaying remaining content when stream appears to be ending
        const finishTime = Math.min(1000, Math.max(300, remainingChars * 15)); // 300ms to 1s range
      ...
      if (remainingChars > 200) {
        charsPerSecond = 75; // Very large buffer - faster reveal
      } else if (remainingChars > 100) {
        charsPerSecond = 60; // Large buffer
      } else if (remainingChars > 50) {
        charsPerSecond = 45; // Medium buffer
      } else {
        charsPerSecond = 35; // Small buffer - more noticeable animation
      }
    }
  ...
  useEffect(() => {
    ...
    if (newContent.length > 0) {
      ...
      streamingPauseTimeoutRef.current = setTimeout(() => {
        // If we haven't received new content for a while, assume streaming is pausing/ending
        isPausingRef.current = true;
        
        // Calculate estimated time when animation should complete
        // We want to finish revealing any buffered content within ~1-2 seconds
        const remainingChars = contentBufferRef.current.length - lastVisibleLengthRef.current;
        const estTimeToReveal = Math.min(2000, remainingChars * 20); // Max 2 seconds, min 20ms per char
        streamEndTimeEstimateRef.current = performance.now() + estTimeToReveal;
      }, 500); // 500ms pause detection
      
      // Track chunk metrics - CRITICAL for dynamic animation pacing
    ...
    contentBufferRef.current = content;
  }, [content, isStreaming, visibleContent]);
  ...
  useEffect(() => {
    ...
    const animateReveal = (timestamp: number) => {
      ...
      if (isPausingRef.current && streamEndTimeEstimateRef.current !== null) {
        // We're in the stream-ending phase - pace the reveal to finish by estimated end time
        ...
        // Normal streaming phase - use adaptive speed based on model and chunk patterns
    ...
    if (isFirstRenderRef.current && visibleContent === '') {
      setVisibleContent(content.length > 0 ? content.charAt(0) : '');
      lastVisibleLengthRef.current = visibleContent.length > 0 ? 1 : 0;
  ...
  }, [isStreaming, content, visibleContent]);
  ...
  // Apply very subtle fade effect to make transitions smoother
  return (
    <motion.div
      className="w-full"
