...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content }) => {
  ...
  const CodeBlock: React.FC<CodeBlockProps> = ({ language, children }) => {
    ...
    // ChatGPT-like colors
    const bgColor = theme === 'dark' ? '#2b2b2b' : '#f7f7f8';
    const borderColor = theme === 'dark' ? '#3e3e42' : '#e5e5e5';
    const textColor = theme === 'dark' ? '#f7f7f8' : '#2b2b2b';
    ...
    return (
      <div className="my-5 relative">
        ...
        >
          {/* Language label bar at the top, like ChatGPT */}
          <div
            style={{
              position: 'absolute',
              top: 0,
              ...
              height: '2.1em',
              width: '100%',
              background: theme === 'dark' ? '#23272f' : '#f3f3f3',
              borderBottom: `1px solid ${theme === 'dark' ? '#343541' : '#e5e5e5'}`,
              display: 'flex',
              alignItems: 'center',
              paddingLeft: '1em',
              fontSize: '0.85em',
              fontWeight: 500,
              color: theme === 'dark' ? '#b4bbc7' : '#6e6e80',
              zIndex: 1,
          ...
          <button
            onClick={handleCopy}
            className={cn(
              `absolute top-2 right-2 px-2 py-1 rounded text-xs font-medium transition-all duration-200 flex items-center gap-1.5`,
          ...
          <div style={{ paddingTop: '2.1em' }} /> {/* Spacer for the label bar */}
          <SyntaxHighlighter
